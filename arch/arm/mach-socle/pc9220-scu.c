...
#include <mach/regs-pc9220-scu.h>
...
static u32 socle_scu_base = IO_ADDRESS(SOCLE_APB0_SCU);
...
//	printk("ratio = %x \n",scu_ratio );
	...
	if(socle_scu_sdram_bus_width_status() == SOCLE_SCU_SDRAM_BUS_WIDTH_32)
		printk("CPU = %ld MHz , HCLCK = %ld MHz (32bit SDR) \n",scu_clock/1000000, socle_get_ahb_clock()/1000000);
	else
		printk("CPU = %ld MHz , HCLCK = %ld MHz (16bit SDR) \n",scu_clock/1000000, socle_get_ahb_clock()/1000000);
