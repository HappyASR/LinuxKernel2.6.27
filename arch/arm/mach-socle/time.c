...
unsigned long __init cal_cpu_freq(void);
...
unsigned long __init cal_cpu_freq(void)
{
	...
	timer0->TimerLoad=0xffffffff;
	/* Start counter exactly on falling edge of update flag */
	reg_time = readl(SOCLE_RTC_TIME);

	/* Start timer counter. */
	timer0->TimerControl=0x18a;

	sec = reg_time & 0xf0;
	do {
		reg_time = readl(SOCLE_RTC_TIME);
	}while (sec == (reg_time & 0xf0));

	apb_clock = timer0->TimerValue;
	apb_clock= 0xffffffff -apb_clock;
//	printk("apb_clock1 %lx \n",apb_clock);
	RTC_DIS();
	return (apb_clock);
...
#ifdef CALCULATE_CPU_FREQ
//	printk("APB freq = %d\n",cal_cpu_freq());
#if (defined(CONFIG_ARCH_CDK) || defined(CONFIG_ARCH_PDK_PC9002) || defined(CONFIG_ARCH_P7DK) || defined(CONFIG_ARCH_PDK_PC7210) || defined(CONFIG_ARCH_SCDK)  || defined(CONFIG_ARCH_MSMV) || defined (CONFIG_ARCH_PDK_PC9220))
//socle_scu_mpll_set(SOCLE_SCU_CPU_CLOCK_266);//channing
get_pll_clock();
#else
	cal_cpu_freq();
#endif
