...
static char __initdata banner[] = "SQ GPIO Service, (c) 2010 SQ Corp.\n";
...
bad:
	printk("SQ GPIO: trying to allocate pin[%d] fail!\n", pin);
	return -EINVAL;
busy:
	return -EBUSY;
}
...
socle_free_gpio_irq(int pin, void *dev_id)
{
	...
	if (irq->active) {
		printk("SQ GPIO: freeing active pin[%d]\n", pin);
		irq->active = 0;
...
bad:
	printk("SQ GPIO: trying to free pin[%d] fail!\n", pin);
}
...
bad:
	printk("SQ GPIO: trying to enable free pin[%d] fail!\n", pin);
	BUG();
}
...
bad:
	printk("SQ GPIO: trying to disable free pin[%d] fail!\n", pin);
	BUG();
}
...
socle_init_gpio_irq(void)
{
	...
	socle_platform_gpio_irq_init(socle_gpio);

#ifdef SOCLE_GPIO_WITH_INT
	socle_gpio[0].irq_num = 0;
	ret = request_irq(socle_gpio[0].irq, socle_gpio_isr_gp0, IRQF_SHARED, socle_gpio[0].name, socle_gpio);
	if (ret) {
	...
	ret = request_irq(socle_gpio[1].irq, socle_gpio_isr_gp1, IRQF_SHARED, socle_gpio[1].name, socle_gpio);
	if (ret) {
		printk("Error! Fail to request irq(%d)\n", socle_gpio[1].irq);
		return ret;
	}
...
//	socle_gpio_irq_proc_entry = create_proc_entry("sq_gpio_irq", S_IRUGO | S_IFREG, &proc_root);
	socle_gpio_irq_proc_entry = create_proc_entry("sq_gpio_irq", S_IRUGO | S_IFREG, NULL);
...
core_initcall(socle_init_gpio_irq);
