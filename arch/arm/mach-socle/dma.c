...
socle_request_dma(u32 ch, const char *dev_id, struct socle_dma_notifier *notifier)
{
	...
	return ret;
bad_dma:
	printk(KERN_ERR "SQ DMA Channel%d.%s: trying to allocate DMA\n", ch, dma->dma_name);
	return -EINVAL;
busy:
	return -EBUSY;
}
...
socle_free_dma(u32 ch)
{
	...
	if (ch >= SOCLE_MAX_DMA_CHANNELS || !dma->ops)
		goto bad_dma;
	if (dma->active) {
		printk(KERN_ERR "SQ DMA Channel%d.%s: freeing active DMA\n", ch, dma->dma_name);
	...
	printk(KERN_ERR "SQ DMA Channel%d.%s: trying to free free DMA\n", ch, dma->dma_name);
	return;
bad_dma:
	printk(KERN_ERR "SQ DMA Channel%d.%s: trying to free DMA\n", ch, dma->dma_name);
}
...
socle_enable_dma(u32 ch)
{
	...
	return;
free_dma:
	printk(KERN_ERR "SQ DMA Channel%d.%s: trying to enable free DMA\n", ch, dma->dma_name);
	BUG();
}
...
socle_disable_dma(u32 ch)
{
	...
	return;
free_dma:
	printk(KERN_ERR "SQ DMA Channel%d.%s: trying to disable free DMA\n", ch, dma->dma_name);
	BUG();
}
...
socle_set_dma_transfer_count(u32 ch, u32 cnt)
{
	...
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering transfer count "
		       "while channel active\n", ch, dma->dma_name);
	socle_dma_channel[ch]->tx_cnt = cnt;
	return;
free_dma:
	printk(KERN_ERR "SQ DMA Channel%d.%s: trying to access free DMA\n", ch, dma->dma_name);
	BUG();
}
...
socle_set_dma_source_address(u32 ch, u32 addr)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering source address "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_destination_address(u32 ch, u32 addr)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering destination address "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_source_direction(u32 ch, int dir)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering source direction "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_destination_direction(u32 ch, int dir)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering destination direction "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_burst_type(u32 ch, int burst)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering burst type "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_data_size(u32 ch, int size)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering data size "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_mode(u32 ch, int mode)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering dma mode "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_ext_hdreq_number(u32 ch, u32 num)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering external hardware request number "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_fly_operation(u32 ch, u32 fly_op)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering fly operation "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_slice_count(u32 ch, u32 cnt)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering slice count "
		       "while channel active\n", ch, dma->dma_name);
...
socle_set_dma_buffer_size(u32 ch, u32 size)
{
	...
	if (!dma->lock)
		goto free_dma;
	if (socle_dma_channel[ch]->active) 
		printk(KERN_ERR "SQ DMA Channel%d.%s: altering buffer size "
		       "while channel active\n", ch, dma->dma_name);
...
//	socle_dma_proc_entry = create_proc_entry("sq_dma", S_IRUGO | S_IFREG, &proc_root);
	socle_dma_proc_entry = create_proc_entry("sq_dma", S_IRUGO | S_IFREG, NULL);
