/* socle_mac.c: ethernet driver for the SQ MAC core with AMBA interface.
...
#define DRV_NAME	"sq_mac"
#define DRV_VERSION	"v1.0"
#define DRV_RELDATE	"FEB 23, 2004"
#define DRV_BUSMODE	"Embedded AMBA"

#define	CONFIG_SOCLEMAC_BASE_ADDR1 IO_ADDRESS(SOCLE_AHB0_MAC0)
#define	CONFIG_SOCLEMAC_INTV1 IRQ_MAC0

#ifdef SOCLE_AHB0_MAC1
#define CONFIG_SOCLEMAC_BASE_ADDR2 IO_ADDRESS(SOCLE_AHB0_MAC1)
...
/* SOCLE_MAC:
	...
	depending on how the hardware is wired and configured.  The SQ MAC
...
/* SOCLE_MAC:
	...
	we define an explict interrupt enable bitmap for the SQ MAC Modules.
...
static int	index= 0;		//20080416 leonid+ for probe function
								

MODULE_AUTHOR("SQ_MAC.c:");
MODULE_DESCRIPTION("SQ MAC AMBA ethernet driver");
MODULE_LICENSE("GPL");
#ifdef MODULE_PARM_DESC
//MODULE_PARM_DESC(mac_max_dev, 
//				 "SQ MAC driver maximum devices to install");
//MODULE_PARM_DESC(mac_active_dev, 
//				 "SQ MAC driver install from which one");
//20080416 leonid- for probe modify
/*
MODULE_PARM_DESC(mac_amba_base_address, 
				 "SQ MAC driver AMBA base address");
MODULE_PARM_DESC(mac_amba_irq, 
				 "SQ MAC driver IRQ number");	
*/
				 			 
MODULE_PARM_DESC(mac_debug_level, 
				 "SQ MAC driver message level (0-31)");
MODULE_PARM_DESC(max_interrupt_work,
				 "SQ MAC driver maximum events handled per interrupt");
MODULE_PARM_DESC(rx_copythreshold,
				 "SQ MAC threshold in bytes for copy-only-tiny-frames");
MODULE_PARM_DESC(csr0,
				 "SQ MAC driver CSR0 setting");				 				 
MODULE_PARM_DESC(multicast_filter_limit,
				 "SQ MAC threshold for switching to Rx-all-multicast");
MODULE_PARM_DESC(options,
				 "SQ MAC force transceiver type or fixed speed+duplex");
MODULE_PARM_DESC(mtu, 
				 "SQ MAC Jumbo MTU for interfaces");
#endif
...
/* The SQ MAC Rx and Tx buffer descriptors. */
...
/* SOCLE_MAC:
	The SQ MAC device driver private struct.
*/
...
/* A list of all installed SQ MAC devices. */
...
static int socle_mac_start_link(struct net_device *dev)
{
	...
	// For MII Phy chip section  
	{
		...
		//+arthur rmii
//		printk("SQ_mac_start_link %s: MII PHY #%d : config<%4.4x> status<%4.4x> advertising<%4.4x>.\n",
...
//					udelay(100);
				...
				if (status & (RDESC0_FIFOERR)) 			
				{
					...
					// SQ MAC when FIFO error must re-start Rx
					outl(0, ioaddr + CSR_OFFSET(CSR2));		/* Rx poll demand */
					//Sherlock 
...
//	static unsigned char 		last_phys_addr[6] = {0x00, 'S', 'O', 'c', 'l', 'e'};
	static unsigned char 		last_phys_addr[2][6] = {{0x00, 'S', 'O', 'c', 'l', 'e'},
		{0x00, 'S', 'O', 'C', 'l', 'e'}};
	int i;
	
	
	printk("SQ_ether_probe\n");
	...
	{
		printk(MAC_KERN_ERR "The SQ MAC moudle (%d) at 0x%08x is not functioning.\n",
					index, base);
		return EIO;
	}
...
//		   dev->name, "SQ MAC core w/AMBA", base, irq);
	...
	if(mac_srom_pt == NULL)
	{	
		memcpy(dev->dev_addr,&last_phys_addr[index],6); //+Peter
		last_phys_addr[index][0] += 2; 	
...
/* cleanup_mac() */
static int __devexit ldk_ether_remove(struct platform_device *pdev)
{
...
static struct platform_driver socle_ether_driver = {
	...
	.remove		= __devexit_p(ldk_ether_remove),
//	.suspend	= ldk_ether_suspend,
//	.resume		= ldk_ether_resume,
	.driver		= {
		.name	= "sq_ether",	//20080416 leonid+ for device naming with devices.c
		.owner	= THIS_MODULE,
	},
};
