...
* Author        : cyli
* Description   : Socle INR KeyPad Driver
* 
...
struct input_dev *pdk_kpd;
EXPORT_SYMBOL(pdk_kpd);
unsigned char *pdkkpd_keycode;
EXPORT_SYMBOL(pdkkpd_keycode);
...
static char __initdata banner[] = "SQ INR KeyPad Driver, (c) 2008 SQ Corp.\n";
...
gpio_kpd_init(void)
{
	...
	// normal mode
	socle_gpio_test_mode_en(PC, 0);

	// single low edge trigger (row - PC0, PC1, PC2, PC3)
	socle_gpio_set_interrupt_sense_with_mask(PC, 0x0, 0xf);
	socle_gpio_set_interrupt_both_edges_with_mask(PC, 0x0, 0xf);
	socle_gpio_set_interrupt_event_with_mask(PC, 0x0, 0xf);

	// set row as input (row - PC0, PC1, PC2, PC3)
	(void)socle_gpio_get_value_with_mask(PC, 0xf);

	// write col pins as 0 (col - PC4, PC5, PC6, PC7)
	socle_gpio_set_value_with_mask(PC, 0x0, 0xf0);

	// enable all interrupt (row - PC0, PC1, PC2, PC3)
	socle_gpio_set_interrupt_mask_with_mask(PC, 0xf, 0xf);
...
pdk_keypad_input_report_key(void *param)
{
	...
	while (1) {
		...
		// read col pins (col - PC4, PC5, PC6, PC7)
		tmp2 = socle_gpio_get_value_with_mask(PC, 0xf0);
		...
		// read col pins (col - PC4, PC5, PC6, PC7)
		tmp = socle_gpio_get_value_with_mask(PC, 0xf0);
		KPD_DBG("tmp = 0x%x\n", tmp);
...
pdk_keypad_isr(int irq, void *dev)
{
	...
	// read row pins
	tmp1 = socle_gpio_get_interrupt_status_with_port(PC);

	if (0xf & tmp1) {
		// disable all interrupt (row - PC0, PC1, PC2, PC3)
		socle_gpio_set_interrupt_mask_with_mask(PC, 0x0, 0xf);
	} else {
		return IRQ_HANDLED;
	}
...
MODULE_DESCRIPTION("SQ INR KeyPad Driver");
MODULE_LICENSE("GPL");
