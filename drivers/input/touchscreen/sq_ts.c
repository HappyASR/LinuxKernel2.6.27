...
//	printk("do_touch \n");
	...
	absy = buf[0] & 0x3ff;		/* use 10 bit resolution */
	tsc2000_read_data(&touchscreen, PG0, 1, buf, 1);	/* page 0, regaddr=0 */
	absx = buf[0] & 0x3ff;		/* use 10 bit resolution */
...
static int __devinit ts_tsc2000_probe(struct spi_device *spi)
{
	...
	/* Setup TSC2000 */
	tsc2000_write_data(&touchscreen, PG1, 0x4, 0xbb00);

	tsc2000_write_data(&touchscreen, PG1, 0x5, 0x3f);
