...
{
	dev_dbg(host->dev, "sq_spi_get_rx_8()\n");
...
{
	dev_dbg(host->dev, "sq_spi_set_tx_8()\n");
...
{
	dev_dbg(host->dev, "sq_spi_set_rx_16()\n");
...
{
	dev_dbg(host->dev, "sq_spi_get_tx_16()\n");
...
socle_spi_calculate_divisor(struct spi_device *spi, u32 clk)
{
	...
	dev_dbg(host->dev, "sq_spi_calculate_divisor()\n");
...
socle_spi_chipselect(struct spi_device *spi, int value)
{
	...
	dev_dbg(host->dev, "sq_spi_chipselect()\n");
...
socle_spi_setup_transfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	dev_dbg(host->dev, "sq_spi_setup_transfer()\n");
	//printk("SQ_spi_setup_transfer()\n");
	//socle_spi_base = IO_ADDRESS(host->io_area->start);		//leonid 20090420 del for no use socle_spi_base
...
socle_spi_txrx_bufs(struct spi_device *spi, struct spi_transfer *t)
{
	...
	dev_dbg(host->dev, "sq_spi_txrx_bufs()\n");
...
socle_spi_remove(struct platform_device *pdev)
{
	...
	dev_dbg(host->dev, "sq_spi_remove()\n");
	if (!host)
		return -1;
...
MODULE_DESCRIPTION("SQ SPI Driver");
