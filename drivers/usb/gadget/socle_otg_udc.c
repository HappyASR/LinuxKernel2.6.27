...
#define	DRIVER_DESC_OTG	"SQ High-Speed USB OTG Controller driver"
//leonid+ for OTG
static const char driver_name_otg[] = "sq_otg";
static const char driver_desc_otg[] = DRIVER_DESC_OTG;

#endif

#define	DRIVER_DESC	"SQ High-Speed USB SOC Device Controller driver"
#define	DRIVER_AUTHOR	"Leonid"
#define	DRIVER_VERSION	"DEC 12, 2008"

#define	DMA_ADDR_INVALID	(~(dma_addr_t)0)

static const char driver_name[] = "sq_otg_udc";
...
static int socle_otg_vbus_draw(struct usb_gadget *gadget, unsigned mA)
{
	...
	//printk("SQ_otg_vbus_draw -- CONFIG_USB_OTG\n");	
...
static irqreturn_t socle_otg_udc_irq(int irq, void *_udc)
{
	...
	//printk("SQ_otg_udc_irq : 0x%08x\n", ioread32(IO_ADDRESS(SOCLE_AHB0_OTG) + 0x144));
	//printk("SQ_otg_udc_irq\n");
...
static int socle_otg_udc_probe(struct platform_device *pdev)
{
	...
	dr_regs = (void *)IO_ADDRESS((u32)res->start); 			//leonid fix for OTG warning message
...
unsigned long otg_sc = IO_ADDRESS(SOCLE_AHB0_OTG) + 0x1a4;
...
static int socle_otg_probe(void)
{
	...
	ret = request_irq(IRQ_OTG, socle_otg_irq, IRQF_SHARED, driver_name_otg, otg_dev);
...
{
	if(probe_a){
		printk("SQ_otg_remove : remove A\n");	
		//usb_ehci_socle_remove(hcd, dev);
	}else{
		printk("SQ_otg_remove : remove B\n");	
		udc_exit();
	}
