...
/* 	if (DEBUG) fprintf(stderr, "Root found at r = %d, (255-r) = %d\n", r, (255-r)); */
	if (DEBUG) printk(KERN_INFO"Root found at r = %d, (1023-r) = %d\n", r, (1023-r));
      }
  }
}
			     ...
			     int csize)
{
  ...
  if ((NErrors <= NPAR) && NErrors > 0) { 
	...
	if (DEBUG) printk(KERN_INFO"Error loc i=%d outside of codeword length %d\n", i, csize);
	return(0);
      }
    }
...
/* 	denom ^= gmult(Lambda[j], gexp[((255-i)*(j-1)) % 255]); */
	denom ^= gmult(Lambda[j], gexp[((1023-i)*(j-1)) % 1023]);
      ...
      if (DEBUG) printk(KERN_INFO"Error magnitude %#x at loc %d\n", err, csize-i);
...
/*       codeword[csize-i-1] ^= err; */
    ...
    if (DEBUG && NErrors) printk(KERN_INFO"Uncorrectable codeword\n");
    return(0);
  }
}
...
correct_errors_hw_ecc(unsigned char *codeword, int csize)
{
     ...
     if ((NErrors <= NPAR) && (NErrors > 0)) {
	  ...
	  for (r = 0; r < NErrors; r++) {
	       if (ErrorLocs[r] >= csize) {
		    if (DEBUG)
			 printk(KERN_INFO"Error loc i = %d outside ot codeword length %d\n", i, csize);
		    return 1;
	       }
	  }
	  ...
	  for (r = 0; r < NErrors; r++) {
	       ...
	       err = gmult(num, ginv(denom));
	       if (DEBUG)
		    printk(KERN_INFO"Error magnitue %#x at loc %d\n", err, csize-i-1);
	       ...
	       codeword[csize-i-1] ^= (unsigned char)err;
	       if (DEBUG)
		    printk(KERN_INFO"Corrected value is %x\n", codeword[csize-i-1]);
	  }
	  return 0;
     } else {
	  if (DEBUG && NErrors)
	       printk(KERN_INFO"Uncorrectable codeword\n");
	  return 1;
     }
}

EXPORT_SYMBOL_GPL(correct_errors_hw_ecc);
