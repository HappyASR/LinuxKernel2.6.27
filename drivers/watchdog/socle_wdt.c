...
#define WDTDBUG(fmt, args...) printk( KERN_DEBUG "SQ_WDT : " fmt, ## args)
#else
...
static int socle_wdt_keepalive(void)
{
	...
	WDTDBUG("sq_wdt_keepalive margin = %x\n",tmr_margin);
	return 0;
}
...
#if (defined(CONFIG_ARCH_CDK) || defined(CONFIG_ARCH_PDK_PC9002) || defined(CONFIG_ARCH_SCDK))
	//init WDT Reset
	writel ((readl(SOCLE_CHEETAH_SCU_BASE + CHEETAH_SCU_DEVCON) | SCU_WDT_RESET),(SOCLE_CHEETAH_SCU_BASE + CHEETAH_SCU_DEVCON));
#endif
...
static char banner[] __initdata = KERN_INFO "SQ Watchdog Timer, (c) 2006 SQ Corp.\n";
...
MODULE_DESCRIPTION("SQ Watchdog Device Driver");
MODULE_LICENSE("GPL");
