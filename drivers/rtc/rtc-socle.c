...
socle_rtc_probe(struct platform_device *pdev)
{
	...
	int ret;
#ifndef CONFIG_ARCH_SOCLE5
	struct rtc_time tm;
	int check;
#endif

/*	20080324 leonid+ for pdk-pc7210 RTC initinal	*/
#if defined(CONFIG_ARCH_LDK5) || defined(CONFIG_ARCH_P7DK)  || defined(CONFIG_ARCH_PDK_PC7210) || defined(CONFIG_ARCH_PDK_PC9220)
	u32 check_t;
        RESET_RTC_CIRCUIT();

/*	check if time has work	*/
	check_t = ioread32(SOCLE_RTC_TIME);
	msleep(1);
	//CY T. modify for Warning message 20081224
	if(check_t == ioread32(SOCLE_RTC_TIME)){
...
//        }
	...
	RTCDBUG("irq: %x\n", IRQ_RTC);
	ret = request_irq(IRQ_RTC, socle_rtc_irq,
//			  IRQF_DISABLED,  "socle-rtc", pdev);
	...
	socle_rtc_base = (void *) SOCLE_VA_RTC;
...
#ifdef CONFIG_PM
/* SQ RTC Power management control */


//static int ticnt_save;

static int
socle_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
	pr_debug("leonid : socle_rtc_suspend \n");
	return 0;
}

static int
socle_rtc_resume(struct platform_device *pdev)
{
	pr_debug("leonid : socle_rtc_resume \n");
	return 0;
}
#else
#define socle_rtc_suspend NULL
#define socle_rtc_resume  NULL
#endif
...
static char __initdata banner[] = "SOCLE-SOCLE RTC, (C) 2006 SQ Corp.\n";
...
//	return platform_driver_register(&socle_rtcdrv);
	return platform_driver_probe(&socle_rtcdrv, socle_rtc_probe);
}
...
MODULE_DESCRIPTION("SQ SQ RTC Driver");
MODULE_LICENSE("GPL");
