...
socle_sdmmc_send_command(struct socle_sdmmc_host *host, struct mmc_command *cmd)
{
	...
	if (data) {
		...
		host->blk_idx = 0;
		host->buf_idx = 0;
		host->buf = kmap_atomic(sg->page, KM_BIO_SCR_IRQ) + sg->offset;
...
socle_sdmmc_dma_read_data(struct socle_sdmmc_host *host)
{
	...
	if ((host->buf_idx << 2) == sg->length) {
		...
		sg = &data->sg[host->cur_sg_idx];
		kunmap_atomic(host->buf, KM_BIO_SRC_IRQ);
		host->buf = kmap_atomic(sg->page, KM_BIO_SRC_IRQ) + sg->offset;
		host->buf_idx = 0;
	...
	socle_set_dma_destination_address(DMA_SDMMC_CH, host->dma_addr);
...
socle_sdmmc_dma_write_data(struct socle_sdmmc_host *host)
{
	...
	if ((host->buf_idx << 2) == sg->length) {
		...
		host->buf = kmap_atomic(sg->page, KM_BIO_SRC_IRQ) + sg->offset;
		host->buf_idx = 0;
		dev_dbg(mmc_dev(host->mmc), "sg[%d]: buffer = %p, offset = %d, length = %d\n", host->cur_sg_idx, host->buf, sg->offset, sg->length);
	}
	for (i = 0; i < (data->blksz >> 2); i++)
		host->dma_buf[i] = swab32(host->buf[host->buf_idx++]);
	...
	socle_disable_dma(DMA_SDMMC_CH);
	socle_set_dma_mode(DMA_SDMMC_CH, SOCLE_DMA_MODE_SW);
	socle_set_dma_source_address(DMA_SDMMC_CH, host->dma_addr);
...
socle_sdmmc_dma_notifier_complete(void *_host)
{
	...
	if (data->flags & MMC_DATA_READ) {
		...
		for (i = 0; i < (data->blksz >> 2); i++)
			host->buf[host->buf_idx++] = swab32(host->dma_buf[i]);
...
socle_sdmmc_probe(struct platform_device *pdev)
{
	...
	host->dma_buf = dma_alloc_coherent(mmc_dev(host->mmc), DMA_BUF_SIZE, &host->dma_addr,
					   GFP_KERNEL);
...
socle_sdmmc_remove(struct platform_device *pdev)
{
	...
	dma_free_coherent(mmc_dev(host->mmc), DMA_BUF_SIZE, host->dma_buf, 
			  host->dma_addr);
